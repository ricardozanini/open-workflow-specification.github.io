...
At the heart of our discussions was the proposed A2A call task (see [issue #1102](https://github.com/open-workflow-specification/specification/issues/1102)). Huawei’s team has already developed a proof-of-concept based on the v1.0.0 [HTTP call task](https://github.com/open-workflow-specification/specification/blob/main/dsl-reference.md#http-call), demonstrating how agents can be orchestrated through standard workflows. While the POC works, relying solely on raw HTTP calls proved verbose and introduced complexity—especially for operations like exchanging vCards.
